...
data = ["~" * (n_columns + 8) for _ in range(4)] + ["~~~~" + row + "~~~~" for row in raw] + ["~" * (n_columns + 8) for _ in range(4)]
...
for row in range(n_rows):
    for col in range(n_columns):
        ...
        if n in('XMAS', 'SAMX'): cnt += 1
        if nw in('XMAS', 'SAMX'): cnt += 1
        if ne in('XMAS', 'SAMX'): cnt += 1
        if e in('XMAS', 'SAMX'): cnt += 1
        if w in('XMAS', 'SAMX'): cnt += 1
        if s in('XMAS', 'SAMX'): cnt += 1
        if sw in('XMAS', 'SAMX'): cnt += 1
        if se in('XMAS', 'SAMX'): cnt += 1
...
for row in range(n_rows):
    for col in range(n_columns):
        ...
        if nw in ('MAS', 'SAM') and ne in ('MAS', 'SAM'): cnt += 1
